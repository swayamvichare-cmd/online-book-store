...
async function fetchWishlist() {
  ...
  try {
    const res = await fetch(`${API}/wishlist?user_id=${currentUser.uid}`);
...
async function removeFromWishlist(bookId) {
  ...
  try {
    const res = await fetch(`${API}/wishlist?user_id=${currentUser.uid}&book_id=${bookId}`, {
      method: "DELETE"
    });
...
async function fetchOrders() {
  ...
  try {
    const res = await fetch(`${API}/orders?user_id=${currentUser.uid}`);
